...
final class SetupImControllerTest: XCTestCase {
    ...
    func testSeedDefaultIMsCompletesWithoutError() async throws {
        ...
        await MainActor.run { controller.seedDefaultIMs(view: mock) }
        try await Task.sleep(nanoseconds: 500_000_000)

        await MainActor.run {
            XCTAssertFalse(mock.progressCalls.isEmpty, "Expected at least one progress callback")
            XCTAssertEqual(mock.errors.count, 0)
        }
    }

    func testSeedDefaultIMsCallsRefreshImList() async throws {
        let (url, db) = try makeDB()
        defer { try? FileManager.default.removeItem(at: url) }
        let mock = await MockSetupImView()
        let controller = await LimeIME.SetupImController(
        ...
        await MainActor.run {
            XCTAssertGreaterThanOrEqual(mock.refreshCount, 1)
        }
    }
    ...
    // MARK: - Progress manager integration

    func testProgressManagerDismissedAfterSeed() async throws {
        let (url, db) = try makeDB()
        defer { try? FileManager.default.removeItem(at: url) }
        ...
        await MainActor.run { controller.seedDefaultIMs(view: nil) }
        try await Task.sleep(nanoseconds: 600_000_000)

        await MainActor.run {
            XCTAssertFalse(progress.isVisible, "Progress should be dismissed after seed")
        }
    }

    // MARK: - Main thread callback

    func testSeedCallbacksOnMainThread() async throws {
        let (url, db) = try makeDB()
        defer { try? FileManager.default.removeItem(at: url) }

        class ThreadCaptureMock: MockSetupImView {
            var capturedThread: Thread?
            override func refreshImList() {
        ...
        let controller = await LimeIME.SetupImController(
            dbServer: LimeIME.DBServer(_testDatasource: db), prefs: LimeIME.LIMEPreferenceManager(defaults: UserDefaults(suiteName: UUID().uuidString)!),
            progress: LimeIME.ProgressManager()
        )

        await MainActor.run { controller.seedDefaultIMs(view: threadMock) }
        try await Task.sleep(nanoseconds: 600_000_000)

        await MainActor.run {
            if let t = threadMock.capturedThread {
                XCTAssertTrue(t.isMainThread, "refreshImList must be called on main thread")
            }
        }
    }
}
