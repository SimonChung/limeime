...
#
# ；\n：is placed on the asdf row (right of l) by append_semicolon_key,
# freeing the bottom row space for a wider space bar.
...
# `IPAD_BOTTOM_ROW` replaces wholesale on iPad — so those keys would be
# lost). On iPad we want them visible as primary digits to the right of
# `0`, in the order: `... 9 0 - = ⌫`.
#
# Three pieces:
#   1. `harvest_dash_and_equals(source)` — scans the WHOLE source layout
...
#   2. `strip_dash_and_equals(row)` — removes primary `-`/`=` from a
#      kept row so they don't end up duplicated once promoted.
#   3. `place_dash_and_equals_next_to_zero(row, dash_key, eq_key)` —
...
#         primary  : code 61 (`=`)
#         label    : "=\n+"  (top: =, bottom: +)
#         longPressCode: 43 (`+`)  — slides down / long-presses to commit
...
# longPressCode on the `)` key, not as a primary code) — that row's
...
# Codes that represent modifier / non-character keys — skipped when
...
# phonetic_shift _(95/ㄦ) or et_41 '(39/ㄘ)) and gets stripped to keep the
...
# -------------------------------------------------------------------------
# Per-row IM transforms — applied in pipeline order for 4-row IM layouts.
...
def augment_im_digit_row(row, digit_dash, layout_dash_key=None, eq_key=None):
    ...
    `…\\n—` slot: em-dash primary (8212), ellipsis long-press (8230) —
      replaces the historical =/+ slot in the digit row.
    ⌫: repeating delete so users don't have to reach the top-right corner.
    ...
    # Shifted symbol-shift top row (!@#$%^&*()) — augment as fixed mirror of digit row.
    ...
    if is_symbol_top_row(row):
        ...
        # Insert middle key right of ) (41) — mirrors non-shift position after 0.
        try:
    ...
    if 48 not in codes or 49 not in codes:  # must be a 1–0 digit row
        return row
    ...
    # Layouts with IM-component sublabels (phonetic=ㄅ, dayi=言 …) are left
    # unchanged; plain-number layouts (array_number, cj_number …) get the
    # dual-sliding treatment to match lime_english_number_ipad.
    for k in keys:
        if k.get("sublabel", ""):
            continue  # has IM component hint — don't touch the label
    ...
    if digit_dash is not None:
        # Dash promoted from bottom row — move it right of 0.
        ...
        # No dash anywhere in the layout — use em-dash/ellipsis as fallback.
        keys.insert(ins, {
            "code": 8212, "label": "…\\n—", "sublabel": "",
            "widthPercent": W, "icon": "",
            "isModifier": False, "isRepeatable": False, "isSticky": False,
        ...
        ins += 1
    # else: dash exists but was promoted to zxcv row — nothing added here.
...
def prepend_abc_modifier(row):
    """Detect the asdf row (last primary code ∈ {58, 59, 108, 76}) and prepend abc(-9).

    58 (:) is the shift-layer equivalent of ; — some phonetic/et_41 shift layouts
...
def apply_zxcv_punct_sliding(row):
    ...
    Keys with sublabels (IM components such as phonetic ㄝ/ㄡ/ㄥ) are left unchanged.
    ...
    # Skip if the shift-layer equivalent (<>? = 60/62/63) is already present —
...
def append_semicolon_key(row):
    """Detect the asdf row (last primary code ∈ {58, 59, 108, 76}) and add ；\\n：.

    - Source `;` (59) or `:` (58) without sublabel → upgraded in-place to full-shape
      ；\\n：(65306 tap, 65307 long-press).
    - Source `;`/`:` with sublabel (IM component, e.g. phonetic ㄤ / et_41 ㄗ)
...
def append_fullshape_period(row):
    """Detect the asdf row and append 。\\n，.

    Detection: last primary code ∈ {58, 59, 65306, 108, 76}.
...
def append_enter_key(row):
    ...
    Detection: last primary code = 65292 (the 。key just added).
    """
    if row.get("isBottomRow", False):
        return row
    keys = row["keys"]
...
def ensure_zxcv_shifts(row, extra_keys=None):
    ...
    #     10 normal keys (e.g. phonetic_shift _(95/ㄦ) or et_41 '(39/ㄘ)).
    ...
    #       is already in the row — avoids doubling up IM punct with QWERTY punct.
    if extra_keys:
...
# Used only for lime_wb layouts (very few keys per row — proportional scaling
...
def _is_normal_key(code):
    """Printable character key → gets the fixed 7% normal-key width."""
...
def normalize_im_row_widths(row):
    ...
    Overflow fallback: when normal-key count × 7% > 100% (e.g. hs zxcv row
    with many promoted symbols), all keys share the row equally instead.

    lime_wb is exempt (uses scale_row_to_100 instead — it has very few keys
    ...
    if len(normal) * 7.0 > 100.0:
        # Too many keys to fit at 7% — distribute all keys equally.
...
def _make_bottom_row(is_wb=False):
    ...
    is_wb is kept for API compatibility but no longer customises the row —
...
#   • 4-row (has digit or symbol-shift top row): per-row pipeline applies
...
#   • 3-row (no digit/symbol top row — lime_array, lime_cj family):
#     transform_no_digit_im_rows handles all rows at once.
#
# Width rule: normal (printable) keys → exactly 7%; function/modifier keys
