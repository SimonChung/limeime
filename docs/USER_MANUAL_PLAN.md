...
在 repo root 的 `/manual/` 建立繁體中文使用手冊。手冊給一般使用者閱讀，不放工程架構、測試細節或內部實作追蹤，必要時才連到 `docs/` 技術文件。
...
- 手冊採用一層式檔案結構，主題頁直接放在 `/manual/*.md`，不可再建立只有一個 `overview.md`、`guide.md`、`usage.md` 或 `settings.md` 的子資料夾。
- 使用者可見文字使用繁體中文。
...
舊結構如 `manual/ime-management/overview.md`、`manual/database-management/guide.md` 與 `manual/preferences/settings.md` 會產生無意義的單檔資料夾，後續不得恢復。若某個主題只有一個主頁，請使用 `manual/<topic>.md`。

## 5. 頁面責任

| 頁面 | 責任 | 必須使用的來源/截圖 |
|------|------|--------------------|
| `manual/index.md` | 使用者入口、平台啟用分流、新安裝/換機/排查導引 | `lime_settings_ios_setup.png`, `lime_settings_android_setup.png` |
| `manual/quick-start.md` | iPhone/iPad/Android 啟用鍵盤、狀態提示、舊裝置還原入口、第一次下載或匯入輸入法 | `lime_settings_ios_setup.png`, `lime_settings_android_setup.png`, `lime_settings_*_database.png`, `lime_settings_*_im_list.png`, `lime_settings_*_im_install.png` |
| `manual/troubleshooting.md` | 啟用、碼表、資料庫、語音輸入快速排查 | `LIME_SETTINGS.md`, `ANDROID_VOICE_INPUT.md`, `#88_ISSUE.md` |
| `manual/ime-management.md` | 下載、匯入、啟用、編輯輸入法、分享單一輸入法與關聯字詞 | `lime_settings_*_im_list.png`, `lime_settings_*_im_detail.png`, `lime_settings_*_im_install.png`, `lime_settings_*_record_list.png`, `lime_settings_*_related_list.png` |
| `manual/keyboard-input.md` | 中文、英文、符號、長按、空白鍵、Emoji、iPad 副鍵符號，是使用者實際打字的主要指南 | `ANDROID_IPHONE_KEYBOARD.md`, `KEYBOARD_TYPE.md`, `KEYBOARD_THEME.md`; 必須嵌入並說明代表性的中文鍵盤、英文鍵盤與 Emoji 面板截圖 |
| `manual/preferences.md` | 喜好設定、主題、輸入行為、繁簡、反查、平台注意事項 | `lime_settings_*_preferences.png`, `KEYBOARD_THEME.md` |
| `manual/database-management.md` | 備份、還原、還原預設資料庫、舊版備份 | `lime_settings_*_database.png`, `#88_ISSUE.md` |
| `manual/advanced.md` | 自製輸入法、碼表格式、資料庫進階、學習系統、自訂佈局 | `CIN_LIME_SPEC.md`, `ANDROID_IPHONE_KEYBOARD.md`, `LIMEDB_SPEC.md` |
| `manual/faq.md` | 常見問題與跨頁快速解答 | 全部來源，不可取代各主題頁 |

## 6. 視覺設計規則

CSS 元件定義在 [MANUAL_VISUAL_DESIGNER.md](MANUAL_VISUAL_DESIGNER.md) 與 `assets/css/style.scss`。
...
```powershell
$bad = rg --files manual | Where-Object { $_ -match '[^\x00-\x7F]' }
...
```powershell
$dirs = Get-ChildItem -Path manual -Directory
...
```powershell
Get-ChildItem -Recurse -File manual -Filter *.md |
  ForEach-Object {
...
```powershell
rg -n "<use the banned phrase and wrong-concept regex from the active review workflow>" manual README.md
```
...
- 無短薄頁。
- 無禁止語句與錯誤概念。
- 設定 App 相關頁使用 `lime_settings_*` 截圖。
