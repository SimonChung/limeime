...
2. Add temporary logging around `LIMEService.switchKeyboard(...)`, `initialIMKeyboard()`, and `LIMEKeyboardSwitcher.setKeyboardMode(...)` for:
   - `primaryCode`
   - `mIsChinese`
   - `mIsSymbols`
   - selected keyboard XML id/name
   - active IM code
3. Verify that returning to Cangjie uses `isSymbol=false`, `isIm=true`, and `R.xml.lime_cj` / Cangjie layout.
4. Force a clean redraw when keyboard mode changes from symbol/number to Chinese:
...
5. Avoid broad cache resets unless needed; prefer a targeted mode-state or redraw fix.

## Follow-up questions

The current report includes reproduction steps, screenshot, app version, and Android version. Ask only if needed after attempted reproduction:

- Device model and whether a custom keyboard theme/keyboard size/split-keyboard option is enabled.
- Whether it happens every time after `123` -> Chinese, or only after rotating/changing apps.
- Whether the same issue appears in non-Cangjie keyboards.
...
- If a new APK includes the fix, ask the reporter to retest with the direct APK link and specifically confirm the `123` -> Chinese Cangjie switching path.
