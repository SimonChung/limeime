...
This means the v6.1.13 scrollbar fix was not sufficient for the reporter's device/path, even though it passed local Samsung `SM-A325N` Android 13 verification. Keep the issue open and do not close from the fix commit or APK alone. Automation posted a targeted logcat collection request at https://github.com/lime-ime/limeime/issues/88#issuecomment-4539952465. Next debugging should use the reporter's fresh Samsung A71 / Android 13 v6.1.13 crash stack to compare whether the failing stack is still `NestedScrollView.draw()` / `ScrollBarDrawable.mutate()` or a separate settings-launch path. Do not ask for another APK retest until a newer targeted fix/build exists.
