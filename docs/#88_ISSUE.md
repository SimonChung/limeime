...
Live issue state checked 2026-05-26 after the reporter's v6.1.15 confirmation: issue is closed, labeled `bug`, assigned to `jrywu`, and has `limeimetw` follow-up comments documenting the v6.1.13 negative retest, log analysis, PR #89, v5.2.4-to-newer upgrade/backup guidance, the v6.1.14 scoped retest request, restore/import UI guidance, the concrete `emoji_fts already exists` restore/import crash, and the v6.1.15 scoped retest request. Reporter `peter8777555` then confirmed in https://github.com/lime-ime/limeime/issues/88#issuecomment-4541845116 that `v6.1.15 全部正常 已正常執行無誤`, closed the issue, received a `+1` reaction, and `limeimetw` posted the closing acknowledgement https://github.com/lime-ime/limeime/issues/88#issuecomment-4541914188. The resolved scope is the original Samsung A71 / Android 13 startup/settings failure plus the later preserved v5.2.4/v6.0 restore/import crash path. Earlier reporter evidence included: v6.1.14 could enter the app and showed the current bottom navigation/database-management UI; restoring/importing v5.2.4 settings on v6.1.14 errored and then LIME could no longer open; the attached log showed `emoji_fts already exists`; the reporter still had v5.2.4/v6.0.0 backups and v6.0.2 could import the v5.2.4 backup normally. Earlier reporter follow-up confirmed the crash happened when opening the 「萊姆輸入法」 settings app after upgrading from v6.0.0 to v6.1.12, while another reporter (`ejmoog`) added that uninstalling and reinstalling v6.1.12 also crashed, but installing v6.1.12 over an existing LIME 6 install could still work. Later, `ejmoog` added a narrower compatibility datapoint in https://github.com/lime-ime/limeime/issues/88#issuecomment-4541728673: Samsung A52 5G on Android 15 verified LIME v6.1.13 can be used normally; no additional public follow-up is needed for that datapoint unless new failed evidence appears.
...
Final state: #88 is reporter-confirmed fixed and closed for APK `LIMEHD2026-6.1.15.apk`. The reporter confirmed the v6.1.15 restore/import retest in https://github.com/lime-ime/limeime/issues/88#issuecomment-4541845116 (`v6.1.15 全部正常 已正常執行無誤`) and closed the issue at 2026-05-26T07:59:06Z. Automation added a +1 reaction to that confirmation and posted the closing acknowledgement https://github.com/lime-ime/limeime/issues/88#issuecomment-4541914188. Treat the Samsung A71 / Android 13 settings-entry crash and the preserved v5.2.4/v6.0 backup restore/import `emoji_fts already exists` crash as resolved for the reporter-confirmed Android APK scope. Do not keep an active reporter watch unless the issue is reopened or new failed evidence appears. Internal release-QA notes remain useful for #64 scrollbar regression coverage and broader restore/import tests, but they are not active #88 public follow-up items.



## Reporter-confirmed closure after v6.1.15

Reporter `peter8777555` confirmed in https://github.com/lime-ime/limeime/issues/88#issuecomment-4541845116 that v6.1.15 works normally (`v6.1.15 全部正常 已正常執行無誤`) after the scoped restore/import retest request. The issue was closed by the reporter at 2026-05-26T07:59:06Z. Automation verified the closing acknowledgement https://github.com/lime-ime/limeime/issues/88#issuecomment-4541914188 and the existing `+1` reaction on the reporter confirmation. Remaining notes are internal regression coverage only, not an active reporter watch.
