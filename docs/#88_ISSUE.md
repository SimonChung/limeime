...
Live issue state checked 2026-05-26 after PR #89 merge: issue is closed by maintainer `jrywu`, labeled `bug`, assigned to `jrywu`, and has `limeimetw` follow-up comments documenting the v6.1.13 negative retest, log analysis, PR #89, and v5.2.4-to-newer upgrade/backup guidance. Earlier reporter follow-up confirmed the crash happens when opening the 「萊姆輸入法」 settings app. The original reporter reproduced it after upgrading from LIME v6.0.0 to v6.1.12, while another reporter (`ejmoog`) added that uninstalling and reinstalling v6.1.12 also crashes, but installing v6.1.12 over an existing LIME 6 install can still work.
...
Do not assume the renewed Samsung Settings entry-point failure was fixed by the v6.1.13 scrollbar APK. That second path needed its own targeted metadata change; PR #89 has now merged that source fix, but the current v6.1.13 APK metadata has not been superseded by a newer APK containing PR #89.
...
1. Track the IME settings activity metadata fix/release boundary: the reporter's uploaded v6.1.13 log shows Samsung Settings repeatedly tries to launch `net.toload.main.hd2026/net.toload.main.hd.ui.MainActivity`; PR #89 updates `res/xml/method.xml` on `master` to the declared `.ui.LIMESettings` activity, but the current v6.1.13 APK still predates that metadata fix.
...
- Do not ask for more generic v6.1.13 logs; the current second log was actionable enough for PR #89, and a newer PR-#89-containing APK does not exist yet in `output-metadata.json`.
- Verify both app launch (`.ui.LIMESettings`) and IME activation/input (`.LIMEService`).
- For the newly captured Samsung Settings path, verify a future PR-#89-containing APK so Samsung Settings no longer tries the undeclared `net.toload.main.hd.ui.MainActivity` class.
...
- If maintainer/Jeremy wants reporter validation after a newer targeted Android APK containing PR #89 lands, ask the reporter(s) to retest with the direct APK link and confirm both launching from Android/Samsung input-method settings and opening the app directly.
...
This means the v6.1.13 scrollbar fix was not sufficient for the reporter's device/path, even though it passed local Samsung `SM-A325N` Android 13 verification. PR #89 then fixed the stale IME `android:settingsActivity` metadata and `jrywu` merged it to `master` as `ca2fde90883a` on 2026-05-26, then closed #88 at 2026-05-26T05:10:54Z. Treat the issue as maintainer-closed/source-fix-merged for now; do not post another public retest request or reopen unless the maintainer/reporter/Jeremy asks, or a newer APK containing PR #89 is published and policy/maintainer context calls for a scoped retest. Automation kept a single targeted logcat collection request at https://github.com/lime-ime/limeime/issues/88#issuecomment-4539952465 after duplicate concurrent follow-up comments were removed, then added a mobile-logcat alternative at https://github.com/lime-ime/limeime/issues/88#issuecomment-4539984855.
