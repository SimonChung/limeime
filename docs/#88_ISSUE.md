...
Live issue state checked 2026-05-26 after PR #89 merge, v6.1.14 APK follow-up, reporter screenshots, and automation reply: issue is open, labeled `bug`, assigned to `jrywu`, and has `limeimetw` follow-up comments documenting the v6.1.13 negative retest, log analysis, PR #89, v5.2.4-to-newer upgrade/backup guidance, the post-merge reopen note, the v6.1.14 scoped retest request, and the later restore/import UI guidance. The reporter replied with `可以了`, then provided full screenshots showing the v6.1.14 app opens, bottom navigation is visible, the bottom `設定`, `輸入法`, `喜好設定`, and `資料庫管理` navigation is visible across the submitted screenshots, and the `資料庫管理` screenshot includes restore controls. The remaining open question is not another generic crash retest; it is whether legacy v5.2.4 backup/restore/import actually fails after the reporter follows the v6.x UI path. Earlier reporter follow-up confirmed the crash happens when opening the 「萊姆輸入法」 settings app. The original reporter reproduced it after upgrading from LIME v6.0.0 to v6.1.12, while another reporter (`ejmoog`) added that uninstalling and reinstalling v6.1.12 also crashes, but installing v6.1.12 over an existing LIME 6 install can still work.
...
1. Track reporter follow-up on concrete v5.2.4 backup/restore/import behavior after the v6.x UI guidance. The v6.1.14 screenshots show bottom navigation and database restore controls are visible, so the remaining question is not missing bottom tabs; it is whether a specific backup file, restore action, or input-method import path fails.
...
- Reporter validation was requested for APK `LIMEHD2026-6.1.14.apk`; the reporter responded with 「可以了」 plus a new import/settings UI concern. Wait for confirmation whether the bottom navigation/import path is visible and usable, or for a full screenshot including the bottom edge if it is not.
...
Current interpretation: v6.1.14 appears to have improved the crash/settings-entry path enough for the reporter to enter the app, and the later screenshots do not show a missing bottom-navigation/layout-rendering bug. #88 should remain open only for a concrete legacy v5.2.4 backup/restore/import follow-up if the reporter reports that `資料庫管理` -> `還原資料庫` or the `輸入法` import path fails with a specific file/action/error. This is distinct from the original `ActivityNotFoundException` path and from the v6.1.13 scrollbar crash. Do not ask for another generic logcat unless the reporter reports a new crash or Android/Samsung settings error.