...
1. Track the IME settings activity metadata fix/release boundary: the reporter's uploaded v6.1.13 log shows Samsung Settings repeatedly tries to launch `net.toload.main.hd2026/net.toload.main.hd.ui.MainActivity`; PR #89 updated `res/xml/method.xml` on `master` to the declared `.ui.LIMESettings` activity, and APK `LIMEHD2026-6.1.14.apk` is expected to contain that metadata fix by build provenance. Current follow-up is the reporter’s bottom-navigation/import-path UI question after partial-positive v6.1.14 validation, not another generic log request.
