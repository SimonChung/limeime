...
Local follow-up fix branch `fix/android-restore-emoji-fts-existing` makes emoji FTS initialization idempotent: when `emoji_fts` exists, LIME probes whether it is usable; if the schema is stale/unloadable, it removes the narrow `emoji_fts` schema/shadow rows before recreating the FTS table. The FTS4 fallback also retries once after removing residual `emoji_fts` schema rows if Android SQLite reports `already exists`. New instrumentation coverage builds a restored DB 102 fixture with a stale `emoji_fts` `sqlite_master` row and verifies restore upgrades to DB 104, rebuilds emoji data/IM rows, and recreates a usable FTS4 or FTS5 table depending on runtime SQLite support.
