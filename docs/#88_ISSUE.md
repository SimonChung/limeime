# Issue #88: Samsung Android 13 settings crash and v6.1.14 retest
...
Live issue state checked 2026-05-26 after APK v6.1.14 publication: issue is open, labeled `bug`, assigned to `jrywu`, and has `limeimetw` follow-up comments documenting the v6.1.13 negative retest, log analysis, PR #89, v5.2.4-to-newer upgrade/backup guidance, the post-merge reopen note, and the v6.1.14 retest request. Earlier reporter follow-up confirmed the crash happens when opening the 「萊姆輸入法」 settings app. The original reporter reproduced it after upgrading from LIME v6.0.0 to v6.1.12, while another reporter (`ejmoog`) added that uninstalling and reinstalling v6.1.12 also crashes, but installing v6.1.12 over an existing LIME 6 install can still work.
...
Do not assume the renewed Samsung Settings entry-point failure was fixed by the v6.1.13 scrollbar APK. That second path needed its own targeted metadata change; PR #89 merged that source fix, and Android pre-release APK v6.1.14 now contains the PR #89 settings-entry metadata fix for reporter retest.
...
1. Track the IME settings activity metadata retest: the reporter's uploaded v6.1.13 log shows Samsung Settings repeatedly tries to launch `net.toload.main.hd2026/net.toload.main.hd.ui.MainActivity`; PR #89 updates `res/xml/method.xml` on `master` to the declared `.ui.LIMESettings` activity, and APK v6.1.14 now includes that metadata fix.
...
- Do not ask for more generic v6.1.13 logs; the current second log was actionable enough for PR #89, and APK v6.1.14 now contains the PR #89 metadata fix for targeted retest.
- Verify both app launch (`.ui.LIMESettings`) and IME activation/input (`.LIMEService`).
- For the newly captured Samsung Settings path, verify APK v6.1.14 so Samsung Settings no longer tries the undeclared `net.toload.main.hd.ui.MainActivity` class.
...
- Reporter retest request for APK v6.1.14 has been posted; ask the reporter(s) to confirm both launching from Android/Samsung input-method settings and opening the app directly.
...
Current follow-up state: PR #89 (https://github.com/lime-ime/limeime/pull/89) corrected the IME settings activity metadata and was merged by `jrywu` on 2026-05-26 as `ca2fde90883a`. Android pre-release APK `LIMEHD2026-6.1.14.apk` / version `6.1.14` now includes that metadata fix; `limeimetw` posted the scoped v6.1.14 retest request at https://github.com/lime-ime/limeime/issues/88#issuecomment-4540597661. The issue remains open pending reporter confirmation of both Android/Samsung input-method settings launch and direct app launch on v6.1.14.
