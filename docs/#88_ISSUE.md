...
This means the v6.1.13 scrollbar fix was not sufficient for the reporter's device/path, even though it passed local Samsung `SM-A325N` Android 13 verification. Keep the issue open and do not close from the fix commit or APK alone. Next debugging should collect a fresh crash stack/logcat from the reporter's Samsung A71 on v6.1.13 and compare whether the failing stack is still `NestedScrollView.draw()` / `ScrollBarDrawable.mutate()` or a separate Samsung A71-specific settings-launch path.
