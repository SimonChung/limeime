...
Last updated: 2026-05-21T09:45:00+08:00 (Asia/Taipei)
...
- #71: Community-reported request for a “clear current code/composition” action. Jeremy clarified this is effectively covered by the candidate-bar dismiss button in APK `LIMEHD2026-6.1.7.apk`; a 6.1.7 test/confirmation request was posted in comment `4500567996`. Reporter confirmed 6.1.7 can clear composition via dismiss, but still wants Chinese/English toggle to clear current code in comment `4502867810`. Automation acknowledged this as a remaining usability/enhancement request in comment `4504038809`. Keep open for product/author-team decision; no bug doc needed unless implementation begins.
...
- Summary: New issue #73 was triaged as an Enter-key raw-code commit enhancement and labeled `enhancement`/`Usability`. #58 received a documentation-style answer explaining `.lime` v2 escaping for ASCII `|`. #71 reporter confirmed the 6.1.7 dismiss button clears composition but still wants Chinese/English toggle to clear current code, so it remains an enhancement. #67 has reporter confirmation, +1 reaction, and closing note; no further action unless reopened.
- Changed: Posted #73 acknowledgement comment `4504038619`; posted #58 format answer comment `4504038716`; posted #71 follow-up acknowledgement `4504038809`; labeled #73 and #71 `enhancement`/`Usability`; confirmed #58 labels `question`/`documentation`. APK state remains `LIMEHD2026-6.1.7.apk`.
- Needs: Continue watching #63 for promised video/details and do targeted debugging only after new evidence. Product/author-team decisions are needed for #70, #71, #72, and #73. Watch #58 for sample table or follow-up import failure.
...
- Status: todo
  ...
  Task: Review/plan #70/#71/#72/#73 enhancement requests.
  Context: #70 asks to hide/configure next-code candidates and now has screenshot clarification; #71 asks for Chinese/English toggle to clear current code after confirming dismiss works in 6.1.7; #72 asks to bundle 哈哈倉頡 / raises table source-license questions; #73 asks Enter to commit raw code while Space keeps Chinese candidate commit behavior.
  Next: Decide product direction for #70/#71/#73 and licensing posture for #72; do not bundle external tables without source/author/license verification.
