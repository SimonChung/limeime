...
Last updated: 2026-05-21T13:37:00+08:00 (Asia/Taipei)
...
- #75: New community-reported Android 15 / 6.1.7 Cangjie keyboard rendering bug: after tapping the numeric keyboard once, switching back to Chinese can leave the previous number/symbol keyboard visible behind the Cangjie keys. Classified as `bug` + `Usability`; acknowledgement posted in comment `4505090642`; analysis doc `docs/#75_ISSUE.md` exists. Likely area is Android keyboard switching/redraw state in `LIMEService.switchKeyboard(...)`, `initialIMKeyboard()`, `LIMEKeyboardSwitcher.setKeyboardMode(...)`, and keyboard-view invalidation/background clearing. Keep open for implementation; after a newer APK contains a relevant fix, ask reporter to retest the Cangjie `123` -> Chinese path.
...
- Status: todo
  ...
  Next: Decide product direction for #70/#73/#74 and licensing posture for #72; implement/fix #71 using the same composition-cancel semantics as candidate-bar dismiss; debug #75 keyboard-switch redraw/state cleanup; do not bundle external tables without source/author/license verification.
