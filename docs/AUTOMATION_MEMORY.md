...
- #62: Community-reported Ext-B leading-character related-phrase issue. Reporter confirmed 6.1.4 improved add/search, but runtime related-candidate suggestions remained blank after committing parent `𩼣`. A source-level fix landed in commit `1b615d638830ac1afc20d460105c88ca9a657e6e` (`Fix #62: route CJK Ext-B commits through related-phrase flow`) and is now included in APK `LIMEHD2026-6.1.6.apk`. A 6.1.6 retest request was posted as comment `4498265731`, asking the reporter to verify `𩼣 -> 魚` runtime related candidates. Issue is currently closed by the fix commit; if the reporter still reproduces it, recommend reopening with the 6.1.6 details.
