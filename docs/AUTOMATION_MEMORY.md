...
Last updated: 2026-05-20T20:21:00+08:00 (Asia/Taipei)
...
- #62: Community-reported Ext-B leading-character related-phrase issue. Reporter confirmed 6.1.4 improved add/search, but runtime related-candidate suggestions remained blank after committing parent `𩼣`. A follow-up source-level fix landed in commit `1b615d638830ac1afc20d460105c88ca9a657e6e` (`Fix #62: route CJK Ext-B commits through related-phrase flow`). APK `LIMEHD2026-6.1.6.apk` now contains the follow-up fix, and limeimetw posted a 6.1.6 retest request in comment `4498265731` asking the reporter to verify `𩼣 -> 魚`. The issue had been auto-closed by the fix commit before reporter verification, so Hermes reopened it on 2026-05-20 to keep it pending reporter confirmation. Keep open until the reporter confirms fixed/improved; if they still reproduce it, ask for the tested APK/version plus screenshots/recording and reopen/update the debugging analysis as needed.
- #63: Community-reported Google voice input outputs Simplified and duplicates text. Reporter tested 6.1.2 and said NOT fixed, then provided Google Drive screen-recording/video evidence in comment `4493061495`: `https://drive.google.com/drive/folders/12XSn_FRBjEROuazexf_u4QCJnC2oNpsl`. A source-level fix landed in commit `342b6a3b12665a2267f2c2fa7e79a9ad298e3938` (`fix(android): resolve #63 voice input routing`) and is now included in APK `LIMEHD2026-6.1.6.apk`. A 6.1.6 retest request was posted as comment `4498265930`, asking the reporter to test both the no-microphone-permission Google/system voice path and the LIME inline-dictation path after granting microphone permission. The issue is currently open and pending reporter confirmation. Keep open until the reporter confirms fixed/improved; if they still reproduce it, ask for the tested APK/version plus Android/HyperOS, Google voice settings, and sample phrase details.
...
- Newer observed state supersedes that baseline where noted above: current observed pre-release APK is `6.1.6`; #55 is closed after positive reporter confirmation; #65/#66 are closed as maintainer-created tracking issues; #54 and #64 are closed as completed; #62 and #63 source-level fixes are included in 6.1.6 and retest requests were posted after that APK landed; #67 is tracked as a bug; and #68 is tracked as a maintainer-created cross-platform dismiss/composition bug.
