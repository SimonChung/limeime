...
Last updated: 2026-05-20T18:52:33+08:00 (Asia/Taipei)
...
- #62: Community-reported Ext-B leading-character related-phrase issue. Reporter confirmed 6.1.4 improved add/search, but runtime related-candidate suggestions remained blank after committing parent `𩼣`. A new source-level fix landed in commit `1b615d638830ac1afc20d460105c88ca9a657e6e` (`Fix #62: route CJK Ext-B commits through related-phrase flow`) and auto-closed the issue on 2026-05-20T10:50:21Z. The fix narrows the surrogate-pair commit shortcut to emoji so CJK Ext-B words use the normal related-phrase path. Latest observed APK is still `LIMEHD2026-6.1.5.apk`, so do not ask for retest until a newer APK containing commit `1b615d638830ac1afc20d460105c88ca9a657e6e` is available. After that APK lands, ask the reporter to verify `𩼣 -> 魚` runtime related candidates; if they still reproduce it, recommend reopening with the new APK/version details.
- #63: Community-reported Google voice input outputs Simplified and duplicates text. Reporter tested 6.1.2 and said NOT fixed, then provided Google Drive screen-recording/video evidence in comment `4493061495`: `https://drive.google.com/drive/folders/12XSn_FRBjEROuazexf_u4QCJnC2oNpsl`. A new source-level fix landed in commit `342b6a3b12665a2267f2c2fa7e79a9ad298e3938` (`fix(android): resolve #63 voice input routing`) and auto-closed the issue on 2026-05-20T10:50:21Z. Latest observed APK is still `LIMEHD2026-6.1.5.apk`, so do not ask for retest until a newer APK containing commit `342b6a3b12665a2267f2c2fa7e79a9ad298e3938` is available. After that APK lands, post a concise retest request to the reporter; if they still reproduce it, recommend reopening with the new APK/version details.
...
- Newer observed state supersedes that baseline where noted above: current observed pre-release APK is still `6.1.5`; #55 is closed after positive reporter confirmation; #65/#66 are closed as maintainer-created tracking issues; #54 and #64 are closed as completed; #62 and #63 now have source-level fixes on `master` and were auto-closed by the push, but reporter/device verification should wait for an APK containing those commits; #67 is tracked as a bug; and #68 is tracked as a maintainer-created cross-platform dismiss/composition bug.
